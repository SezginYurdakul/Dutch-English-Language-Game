...
export function loadNextWord() {
  ...
  if (
    gameState.currentWordIndex >= gameState.questionCount ||
    gameState.currentWordIndex >= gameState.shuffledWordList.length
  ) {
    stopTimer();
    showResultsScreen();
    return;
  ...
  const questionWord = isDutchToEnglish
    ? currentWord.dutch
    : currentWord.english;
  const correctOption = isDutchToEnglish
    ? currentWord.english
    : currentWord.dutch;
...
export function checkAnswer(selectedOption, correctOption, button) {
  ...
  const answerLang =
    gameState.gameMode === "dutch-to-english" ? "en-US" : "nl-NL";
...
export function saveGameHistory() {
  ...
  userHistory.push({
    ...
    correct: gameState.correctScore,
    wrong: gameState.wrongScore,
    accuracy: (
      (gameState.correctScore / gameState.questionCount) *
      100
    ).toFixed(2),
